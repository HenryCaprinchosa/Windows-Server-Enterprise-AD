...
![Status: V1.3](https://img.shields.io/badge/Status-V1.3_IIS_Intranet-brightgreen?style=flat-square)
...
7. Navigate to `C:\inetpub\wwwroot` on the server, remove the default files, and create a custom `index.html` for your intranet page.
8. On the Windows 11 VM, ensure the Network Adapter is set to obtain an IP address automatically (DHCP).
9. Join the Windows 11 VM to the domain via Settings -> Accounts -> Access work or school, click Connect, and select "Join this device to a local Active Directory domain". Enter the domain name, followed by credentials with domain-join permissions, then restart.
10. Use `dsa.msc` and `gpmc.msc` on the server to recreate the OU structure and test GPOs by logging into the Windows 11 VM with a domain account.
